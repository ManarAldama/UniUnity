...
public partial class HomePageSks : ContentPage
{
    ...
    public HomePageSks()
	{
        ...
        var sks = TransactionManager.Instance.SksTransaction.GetsksById(ids.SksId); // Assuming you have a method to get the student by ID
        BindingContext = sks;
        labelN.SetBinding(Label.TextProperty, "SKSName");
        ...
        MessagingCenter.Subscribe<AddClub>(this, "clubadded", (sender) =>
        {
            UpdateCollectionView();
        });

        MessagingCenter.Subscribe<ClubsPageSks>(this, "clubUpdated", (sender) =>
        {
            UpdateCollectionView();
        });
    }
    ...
    public HomePageSks(string dateTime, string location, string message, ImageSource imageSource)
    {
    ...
    private async void OnFrameTapped(object sender, EventArgs e)
    {
        ...
        if (sender is Frame frame)
        {
            ...
            if (frame.BindingContext is Event tappedEvent)
            {
                ...
                var alertPage = new HomePageSks(dateTime, location, message, imageSource);
